-- ========================================
-- Advanced Notification System Migration
-- ========================================

-- 1. Add cooldown and notification settings to alerts table
ALTER TABLE alerts 
ADD COLUMN IF NOT EXISTS cooldown_period INTEGER DEFAULT 0,  -- 0=없음, 1=1분, 5=5분, 10=10분, 60=1시간, 1440=1일 (분 단위)
ADD COLUMN IF NOT EXISTS last_triggered_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS sound_enabled BOOLEAN DEFAULT true,
ADD COLUMN IF NOT EXISTS vibrate_enabled BOOLEAN DEFAULT true;

-- Add comment
COMMENT ON COLUMN alerts.cooldown_period IS '중복 알림 제한 시간 (분 단위): 0=없음, 1=1분, 5=5분, 10=10분, 60=1시간, 1440=1일';
COMMENT ON COLUMN alerts.last_triggered_at IS '마지막 알림 발생 시간';
COMMENT ON COLUMN alerts.sound_enabled IS '알림 소리 활성화 여부';
COMMENT ON COLUMN alerts.vibrate_enabled IS '알림 진동 활성화 여부';

-- 2. Create alert_history table for tracking all notifications
CREATE TABLE IF NOT EXISTS alert_history (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  user_id UUID NOT NULL,
  alert_id UUID NOT NULL,
  ticker TEXT NOT NULL,
  timeframe TEXT NOT NULL,
  indicator TEXT NOT NULL,
  condition TEXT NOT NULL,
  trigger_price NUMERIC(12, 2),
  signal_type TEXT NOT NULL,  -- 'buy' or 'sell'
  message TEXT,
  triggered_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  FOREIGN KEY (alert_id) REFERENCES alerts(id) ON DELETE CASCADE,
  FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
);

-- Add indexes for performance
CREATE INDEX IF NOT EXISTS idx_alert_history_user_id ON alert_history(user_id);
CREATE INDEX IF NOT EXISTS idx_alert_history_alert_id ON alert_history(alert_id);
CREATE INDEX IF NOT EXISTS idx_alert_history_triggered_at ON alert_history(triggered_at DESC);
CREATE INDEX IF NOT EXISTS idx_alerts_last_triggered ON alerts(last_triggered_at);

-- Enable RLS on alert_history
ALTER TABLE alert_history ENABLE ROW LEVEL SECURITY;

-- Create policies for alert_history
CREATE POLICY "Users can view their own alert history"
  ON alert_history FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Service can insert alert history"
  ON alert_history FOR INSERT
  WITH CHECK (true);  -- Backend service can insert any record

CREATE POLICY "Users can delete their own alert history"
  ON alert_history FOR DELETE
  USING (auth.uid() = user_id);

-- 3. Add global notification settings to profiles table
ALTER TABLE profiles
ADD COLUMN IF NOT EXISTS notification_sound_enabled BOOLEAN DEFAULT true,
ADD COLUMN IF NOT EXISTS notification_vibrate_enabled BOOLEAN DEFAULT true;

COMMENT ON COLUMN profiles.notification_sound_enabled IS '전역 알림 소리 설정';
COMMENT ON COLUMN profiles.notification_vibrate_enabled IS '전역 알림 진동 설정';

-- 4. Add notification_sent column to alert_history (for tracking)
ALTER TABLE alert_history
ADD COLUMN IF NOT EXISTS notification_sent BOOLEAN DEFAULT true;

COMMENT ON COLUMN alert_history.notification_sent IS '알림 전송 성공 여부';

-- 5. Drop old triggered_alerts table if exists (replaced by alert_history)
-- DROP TABLE IF EXISTS triggered_alerts CASCADE;

-- Note: Keep triggered_alerts for backward compatibility
-- You can manually drop it after confirming alert_history works well

COMMENT ON TABLE alert_history IS '알림 발생 이력 테이블 - 모든 알림 발생 시 기록';
